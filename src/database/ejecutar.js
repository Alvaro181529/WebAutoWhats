...
async function actualizarMensajes(estado, mensaje, descripcion, intentos, entrega, id,numeroEstado,ciudad) {
  const query =
    "UPDATE `mensajes` SET `estado` = ?, `mensajes` = ?, `observacion` = ?, `intentos` = ?,`entrega` = ?,`ciudad` = ?,`numeroEstado` = ?, `fecha_actualizacion` = CURRENT_TIMESTAMP WHERE `id` = ?";
  try {
    await conn.query(query, [estado, mensaje, descripcion, intentos, entrega,ciudad,numeroEstado, id]); // Asegúrate de tener un valor para `id`
