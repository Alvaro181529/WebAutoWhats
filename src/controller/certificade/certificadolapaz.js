...
const {
  ...
  cerrarSesion,
  contactoLPZC,
} = require("../../whatsapp/certificado/certificadolapaz");
let estado = "";
...
exports.certificade = (req, res) => {
  const api = apiExterno();
  res.json(api);
}
exports.certificadolapazController = (req, res) => {
  const codigo = codigoQRLPZC();
  const contacto = contactoLPZC();
  estado = estadoConexionLPZC();
    ...
    try {
      ...
      if (estado == "conectado") {
        ...
        cron.schedule("* * * * 1-6", async () => {
          ...
          console.log("esta aca en comprobacion ", api)
...
exports.certificadolapazControllerReportes = async (req, res) => {
  const { date } = req.body;
...
exports.certificadolapazControllerMessage = async (req, res) => {
...
exports.certificadolapazControllerAuth = (req, res) => {
  const { pass } = req.body;
  const admin = bpass[9].pass;
  const certificadolapaz = bpass[2].pass; // pass beni
  const passwordAdm = CryptoJS.MD5(admin).toString();
  const password = CryptoJS.MD5(certificadolapaz).toString();
...
exports.NotescertificadolapazController = (req, res) => {
  res.json(mensajesLP);
};
exports.NotesCreatecertificadolapazController = (req, res) => {
...
exports.NotesUpdatecertificadolapazController = (req, res) => {
...
exports.NotesDelatecertificadolapazController = (req, res) => {
  const { id } = req.params;
