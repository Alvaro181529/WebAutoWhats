...
async function comprobacionReenvio() {
    ...
    const menQuery1 =
        "SELECT mensajes.*, packages.TELEFONO ,packages.ESTADO ,packages.VENTANILLA FROM mensajes JOIN packages ON mensajes.id_Telefono = packages.id WHERE mensajes.numeroEstado = 1 AND mensajes.intentos =0 AND packages.ESTADO = 'VENTANILLA' AND CUIDAD='SUCRE' ORDER BY mensajes.fecha_actualizacion ASC LIMIT 200;";
...
async function comprobacionReenvio2() {
    ...
    /* seleccina las mensajes mas antiguos y los envio */
    const menQuery1 =
        "SELECT mensajes.*, packages.TELEFONO ,packages.ESTADO ,packages.VENTANILLA FROM mensajes JOIN packages ON mensajes.id_Telefono = packages.id WHERE mensajes.numeroEstado = 1 AND mensajes.intentos =1 AND packages.ESTADO = 'VENTANILLA' AND CUIDAD='SUCRE' ORDER BY mensajes.fecha_actualizacion ASC LIMIT 200;";
